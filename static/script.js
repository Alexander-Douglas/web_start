...
search.addEventListener("search", () => {
  ...
  fetch("/check-weather", {
    ...
    .then((data) => { // Retour des données JSON en Objet JS
      console.log(data);
      // Localisation
      document.getElementById("fr-name").innerHTML = Boolean(data[0].local_names)?:(Boolean(data[0].local_names.fr)?data[0].local_names.fr.toUpperCase():data[0].name.toUpperCase()):data[0].name.toUpperCase();
